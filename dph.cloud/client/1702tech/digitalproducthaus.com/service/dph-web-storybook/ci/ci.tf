...
terraform {
  backend "s3" {
    ...
    key    = "client/1702tech/digitalproducthaus.com/service/dph-web-storybook/ci/terraform.tfstate"
    region = "eu-west-1"
...
module "ci" {
  ...
  build_job = {
    ...
    environment_variables = concat(data.terraform_remote_state.config.outputs.ci.shared_env_vars, [
      ...
      { key = "RELEASE_ARTEFACT_PATH", value = "dph.client.tests/dph.client.tests.ui.storybook/storybook-static" },
  ...
  deploy_job = {
    ...
    environment_variables = concat(data.terraform_remote_state.config.outputs.ci.shared_env_vars, [
      ...
      { key = "AWS_SSM_PARAMETER_PATHS", value = join(";", [
        data.terraform_remote_state.config.outputs.paths.global,
        data.terraform_remote_state.config.outputs.paths.ci_deploy_web_storybook,
      ]) },
    ])
  }
