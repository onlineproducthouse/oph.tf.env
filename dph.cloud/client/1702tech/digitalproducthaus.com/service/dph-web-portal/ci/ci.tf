...
module "ci" {
  ...
  deploy_job = {
    ...
    environment_variables = concat(local.shared_env_vars, [
      ...
      { key = "WORKING_DIR", value = "./" },
      { key = "BUILD_ARTEFACT_PATH", value = "dph.client.web/dph.client.web.portal/build" },
      { key = "S3_HOST_BUCKET_URL", value = data.terraform_remote_state.config.outputs.test_env.web.portal.host.id },
      { key = "CDN_ID", value = data.terraform_remote_state.config.outputs.test_env.web.portal.cdn.id },
    ])
  }
