...
terraform {
  backend "s3" {
    ...
    key    = "client/1702tech/onlineproducthouse.com/ci/projects/oph_web_storybook/terraform.tfstate"
    region = "eu-west-1"
  ...
  ci = {
    ...
    build_job = {
      ...
      environment_variables = concat(data.terraform_remote_state.config.outputs.config.shared_ci_env_vars, [
        ...
        { key = "RELEASE_ARTEFACT_PATH", value = "oph.client.web/oph.client.web.storybook/build" },
