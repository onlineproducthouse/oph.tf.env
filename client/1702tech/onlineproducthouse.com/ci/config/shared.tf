...
data "terraform_remote_state" "email" {
  ...
  config = {
    bucket = "oph-cloud-terraform-remote-state"
    key    = "client/1702tech/onlineproducthouse.com/shared/email/terraform.tfstate"
    region = "eu-west-1"
  }
}
...
locals {
  ...
  test_user_email_addr_templ = "test-user@${data.terraform_remote_state.shared_networking.outputs.dns.domain_name}"
  ...
  shared = [
    ...
    { id = "shared_portal_user_email_addr", path = local.paths.shared, key = "PORTAL_USER_EMAIL_ADDR", value = "${local.secrets.shared.portal_user_email_addr_prefix}@${data.terraform_remote_state.shared_networking.outputs.dns.domain_name}" },
