...
data "terraform_remote_state" "git_repo_webhook" {
  ...
  config = {
    bucket = "oph-cloud-terraform-remote-state"
    key    = "client/1702tech/onlineproducthouse.com/shared/developer_tools/git_repo_webhook/terraform.tfstate"
    region = "eu-west-1"
  }
}
...
output "config" {
  value = {
    ...
    qa = {
      in_use  = data.terraform_remote_state.qa.outputs.qa.environment.network.in_use
      vpc_id  = data.terraform_remote_state.qa.outputs.qa.environment.network.vpc.id
      subnets = data.terraform_remote_state.qa.outputs.qa.environment.network.subnet_id_list.private
    }
  }
}
