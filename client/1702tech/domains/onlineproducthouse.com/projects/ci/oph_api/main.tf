...
terraform {
  backend "s3" {
    ...
    key    = "client/1702tech/domains/onlineproducthouse.com/projects/ci/oph_api/terraform.tfstate"
    region = "eu-west-1"
    ...
    deploy_job = {
      ...
      environment_variables = concat(data.terraform_remote_state.config.outputs.config.shared_ci_env_vars, [
        ...
        { key = "AWS_SSM_PARAMETER_PATHS", value = join(";", [
          ...
          data.terraform_remote_state.config.outputs.config.paths.ci.deploy.container.api.qa,
