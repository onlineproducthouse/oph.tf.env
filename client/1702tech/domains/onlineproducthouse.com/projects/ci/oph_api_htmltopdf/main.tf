...
terraform {
  backend "s3" {
    ...
    key    = "client/1702tech/domains/onlineproducthouse.com/projects/ci/oph_api_htmltopdf/terraform.tfstate"
    region = "eu-west-1"
...
locals {
  run = data.terraform_remote_state.config.outputs.config.qa.is_running.cloud == true && data.terraform_remote_state.config.outputs.config.qa.is_running.platform == true && data.terraform_remote_state.config.outputs.config.qa.is_running.project.api == true
...
module "ci" {
  ...
  ci = {
    ...
    deploy_job = {
      ...
      environment_variables = concat(data.terraform_remote_state.config.outputs.config.shared_ci_env_vars, [
        ...
        { key = "AWS_SSM_PARAMETER_PATHS", value = join(";", [
          ...
          data.terraform_remote_state.config.outputs.config.paths.ci.deploy.container.htmltopdf.qa,
    ...
    pipeline = {
      ...
      git = {
        branch_names   = local.run == true ? ["dev", "qa"] : ["dev"]
