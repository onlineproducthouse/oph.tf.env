...
terraform {
  backend "s3" {
    ...
    key    = "client/1702tech/domains/onlineproducthouse.com/projects/ci/oph_www_portal/terraform.tfstate"
    region = "eu-west-1"
  ...
  ci = {
    ...
    build_job = {
      ...
      environment_variables = concat(data.terraform_remote_state.config.outputs.config.shared_ci_env_vars, [
        ...
        { key = "TARGET_WORKSPACE", value = "apps/portal" },
    ...
    deploy_job = {
      ...
      environment_variables = concat(data.terraform_remote_state.config.outputs.config.shared_ci_env_vars, [
        ...
        { key = "PROJECT_TYPE", value = "client" },
        { key = "RELEASE_ARTEFACT_PATH", value = "./apps/portal/dist/" },
        { key = "AWS_SSM_PARAMETER_PATHS", value = join(";", [
          data.terraform_remote_state.config.outputs.config.paths.shared,
