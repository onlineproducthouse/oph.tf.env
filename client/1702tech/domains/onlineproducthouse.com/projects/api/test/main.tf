...
locals {
  ...
  aws_autoscaling_group = {
    name = data.terraform_remote_state.platform.outputs.test.platform.compute.auto_scaling_group.name
  }
  ...
  api = [
    {
      ...
      container = {
        ...
        network_mode = "host"
        launch_type  = "EC2"
        cluster_id   = data.terraform_remote_state.platform.outputs.test.platform.compute.cluster_id
...
output "test" {
  value = {
    run         = local.run
    api         = module.test.api
    htmltopdf   = module.test.htmltopdf
    domain_name = data.terraform_remote_state.platform.outputs.test.ssl.api.cert_domain_name
  }
}
