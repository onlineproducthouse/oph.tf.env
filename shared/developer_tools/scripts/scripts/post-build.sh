...
if [[ $CI_ACTION == "build" ]]; then
  ...
  if [[ "$GIT_BRANCH" == "qa" ]]; then
    ...
    if [[ "$PROJECT_TYPE" == "container" ]]; then
      ...
      echo "DKR_IMAGE=$IMAGE_REGISTRY_BASE_URL/$IMAGE_REPOSITORY_NAME:$CODEBUILD_RESOLVED_SOURCE_VERSION" >$WORKING_DIR/$RELEASE_MANIFEST
      # zip $BUILD_ARTEFACT_KEY $RELEASE_MANIFEST

      # aws s3 cp "./$BUILD_ARTEFACT_KEY.zip" "s3://$BUILD_ARTEFACT_STORE"
      # BUILD_ARTEFACT_PATH="./$RELEASE_MANIFEST"
    fi
